...
// Result type for getIntentGuardIntercept
type IntentGuardInterceptResult struct {
	...
	for i, tx := range txs {
		...
		if shouldSimulate {
			...
			if !simResult.Match {
				...
				api.backend.TxSimulationPool().StoreIntentGuardIntercept(txCanonicalID, header.Number.Uint64(), reason)
				statedb.RevertToSnapshot(snapshot)
				log.Info("Dropping tx", "index", i, "txhash", txHash, "canonicalID", txCanonicalID, "reason", reason)
				droppedTxs = append(droppedTxs, &DroppedTxInfo{Hash: txHash, Reason: reason})
			} else {
...
// getIntentGuardIntercept RPC: firewall_getIntentGuardIntercept
// For PoC, we switch to canonicalID to identify IntentGuard Intercept.
// Note: we keep the method name for wire-compatibility but the parameter is canonicalID.
func (api *FirewallAPI) GetIntentGuardIntercept(ctx context.Context, canonicalID common.Hash) (*IntentGuardInterceptResult, error) {
	reason, ok := api.backend.TxSimulationPool().GetIntentGuardIntercept(canonicalID)
	if !ok {
		return &IntentGuardInterceptResult{Found: false, Reason: ""}, nil
	}
	return &IntentGuardInterceptResult{Found: true, Reason: reason.Reason}, nil
}
